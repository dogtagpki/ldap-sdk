...
change_spec_version() {
    CURRENT_VERSION=$(grep "Version: " ldapjdk.spec | grep -Eo '[0-9]+(\.[0-9]+)?(\.[0-9]+)?$')
    CURRENT_PHASE=$(grep "_phase " ldapjdk.spec | grep -E 'alpha|beta' | awk '{print $(NF)}')
    CURRENT_RELEASE_NUMBER=$(grep "release_number " ldapjdk.spec | grep -Eo '[0-9]+(\.[0-9]+)?$')

    echo "Update version to $NEXT_VERSION"
    sed -i "/Version:        /cVersion:          $NEXT_VERSION" ldapjdk.spec

    if [[ "$CURRENT_PHASE" != "$NEXT_PHASE" ]] ; then
        if [ -z "$NEXT_PHASE" ] ; then
            echo "Remove phase"
            sed -i "/_phase /c\#global           _phase" ldapjdk.spec
            ...
            sed -i "/#global           _phase/c\%global           _phase -$NEXT_PHASE" ldapjdk.spec
            echo "Update release_number"
            sed -i "/release_number /c\%global           release_number 0.1" ldapjdk.spec
        else
            echo "Update phase to $NEXT_PHASE"
            sed -i "/_phase /c\%global           _phase -$NEXT_PHASE" ldapjdk.spec
            echo "Update release_number"
